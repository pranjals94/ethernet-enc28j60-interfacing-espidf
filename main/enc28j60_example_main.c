/* ENC28J60 Example
  ...
  This is DHCP (Dynamic ip) adressing code, there must be a DHCP server running in the network to acquire the ip
  Ip address
  Tested on the third party dhcp server app "dhcpsrv2.5.2" running in windows

  use capacitors on the 3.3volt power of enc28j60 module

  if you need to reset(power cycle) both esp32 and the enc28j60 module together(or first enc module and then esp32 or vice versa) to work properly
